...
def label_inference(pred, clone_model, target_grad, label_vals, grad_index):
    pred_losses = [
        torch.nn.CrossEntropyLoss()(pred, cd_label) for cd_label in label_vals
    ]
    pred_grads = [
        torch.autograd.grad(
            loss, clone_model.parameters(), allow_unused=True, retain_graph=True
        )[grad_index]
        for loss in pred_losses
    ]
    grad_losses = [
        torch.nn.MSELoss()(pred_grad, target_grad) for pred_grad in pred_grads
    ]
