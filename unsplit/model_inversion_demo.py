...
if dataset == "mnist":
    trainset = datasets.MNIST(
        "data/mnist", download=True, train=True, transform=transforms.ToTensor()
    )
    testset = datasets.MNIST(
        "data/mnist", download=True, train=False, transform=transforms.ToTensor()
    )
    client, server, clone = MnistNet(), MnistNet(), MnistNet()
elif dataset == "f_mnist":
    trainset = datasets.FashionMNIST(
        "data/f_mnist", download=True, train=True, transform=transforms.ToTensor()
    )
    testset = datasets.FashionMNIST(
        "data/f_mnist", download=True, train=False, transform=transforms.ToTensor()
    )
    client, server, clone = MnistNet(), MnistNet(), MnistNet()
elif dataset == "cifar":
    trainset = datasets.CIFAR10(
        "data/cifar", download=True, train=True, transform=transforms.ToTensor()
    )
    testset = datasets.CIFAR10(
        "data/cifar", download=True, train=False, transform=transforms.ToTensor()
    )
...
for epoch in range(epochs):
    ...
    for images, labels in trainloader:
        ...
        print(
            f"Epoch: {epoch} Loss: {running_loss / len(trainloader)} Acc: {get_test_score(client, server, testset, split=split_layer)}"
        )
print("Done.")
...
for idx, target in enumerate(targetloader):
    ...
    # perform the attack
    result = unsplit.model_inversion_stealing(
        clone,
        split_layer,
        client_out,
        target.size(),
